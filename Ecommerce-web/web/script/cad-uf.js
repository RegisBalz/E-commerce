...
function loadTabela() {
    var URL = 'http://localhost:8080/ecommerce-web/loadTabelaUF';
...
function novoRegistro() {
    ...
    document.querySelector('#uf_nome').focus();
...
function salvarCampos(e) {
    ...
    var formData = {};
    formData['uf_nome'] = oForm.uf_nome.value;
    ...
    var msg = [];
    if (formData.uf_nome.length !== 2 || /^[A-Z]*$/.test(formData.uf_nome) === false) {
    ...
    if (msg.length > 0) {
        ...
        var URL = 'http://localhost:8080/ecommerce-web/saveUF';
        var http = new XMLHttpRequest();
...
function removerCampos(e) {
    if (confirm("Tem certeza que deseja excluir o registro selecionado?")) {
        ...
        var URL = 'http://localhost:8080/ecommerce-web/removeUF';
        var http = new XMLHttpRequest();
